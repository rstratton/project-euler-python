...
direction (up, down, left, right, or diagonally) in the 20x20 grid (number grid
below)?
"""
...
GRID = [[int(num) for num in row.split(" ")] for row in GRID_STR.split("\n")]
...
def v_prod(i, j):
    """
    Return the product of 4 consecutive vertial entries in the grid starting at
    (i, j) and moving down. 'i' indexes rows and 'j' indexes colums, from
    the top-left of the grid.
    """
    if i > 16:
        return -1
...
def dr_prod(i, j):
    ...
    (i, j) and moving down-right. 'i' indexes rows and 'j' indexes colums, from
    the top-left of the grid.
    """
    if j > 16 or i > 16:
        return -1
...
def max_product(i, j):
    """
    Return the largest of the horizontal, vertical, and diagonal prodocts from
    point (i, j).
    """
    return max(h_prod(i, j), v_prod(i, j), dr_prod(i, j), dl_prod(i, j))

def solve():
    return max(max_product(i, j) for i in range(20) for j in range(20))
